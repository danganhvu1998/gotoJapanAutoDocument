...
def autoRecognize(auStudentID):
	dictData = dict()
	# taking input file
	listFileName = os.listdir("autoRecognize/")
	cnt = 0
	print("Choose the input file you want to auto recognize information (number or name): ")
	for file in listFileName:
		cnt=cnt+1
		print(str(cnt)+".",file)
	idFile = input()
	try:
		intIDfile=int(idFile)-1
		inputFileName = listFileName[intIDfile]
	except:
		inputFileName = idFile
		if(inputFileName.strip().lower()=="exit"): return
	dirInput = "autoRecognize/"+inputFileName

	#taking folder form file
	listFileName = os.listdir("data/")
	cnt = 0
	print("Choose the folder contains the form file (number or name): ")
	for file in listFileName:
		cnt=cnt+1
		print(str(cnt)+".",file)
	idFile = input()
	try:
		intIDfile=int(idFile)-1
		formFileFolder = listFileName[intIDfile]
	except:
		formFileFolder = idFile
		if(inputFileName.strip().lower()=="exit"): return

	#taking form file
	listFileName = os.listdir("data/"+formFileFolder)
	cnt = 0
	print("Choose the folder contains the form file (number or name): ")#
	for file in listFileName:
		cnt=cnt+1
	...
	try:
		intIDfile=int(idFile)-1
		formFileName = listFileName[intIDfile]
	except:
		formFileName = idFile
		if(inputFileName.strip().lower()=="exit"): return
	dirForm = "data/"+formFileFolder+"/"+formFileName

	#checking file
	if("docx" in inputFileName and "docx" not in formFileName):
		print("Invalid File: Docx and Xlsx")
		return
	elif("xlsx" in inputFileName and "xlsx" not in formFileName):
		print("Invalid File: Docx and Xlsx")
		return
	
	#taking info	
	if("docx" in inputFileName):
		#docx FIRSR
	...
	while(1):
		reAuStudentID = input("student Name is: "+student["NaMe"]+". Reinput studentID to confirm. studentID: ")
		if(reAuStudentID!=auStudentID):
			print("studentID cannot be confirmed. Reinput studentID or type \"exit\" to exit")
			continue
		elif(reAuStudentID.strip().lower()=="exit"): break
		...
		fhand.close()
		try:
			os.startfile("autoRecognizeInfo.txt")			
		except:
			webbrowser.open("autoRecognizeInfo.txt")
		if (input("Do you wish to upload these information to server(y/n)?").lower()=="y"):
			uploadStudent_AutoRead(auStudentID)
		break
...
def exportFile(exStudentID, exGroup):#rewrite - last one
	...
	desFrom = destination+"data/"+exGroup+"/"
...
while(1):
	strRequest = str(input("(MAIN PAGE) What do you need? ~")).lower().strip()
	if(strRequest=="exit"):
		sys.exit("Bye!")
	elif(strRequest=="author"):
		author()
	elif(strRequest=="reviewall"):
		reviewALL()
		if(input("You can always open ErrLog.txt, want to open it now?(y/n)").lower().strip()=="y"):
			webbrowser.open("ErrLog.txt")
	else:
		if(defStudentInfo(strRequest)==0):
			print("No command fould! List all command:\n\t+reviewall(MASTERPASSWORD required)\n\t+ID_student(ex:2018)\n\t+author\n\t+exit")
		else:
			seeStudent(strRequest)
